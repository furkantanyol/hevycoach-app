...
- Dependencies, and nothing else: `expo`, `expo-router`, `expo-linking`, `expo-constants`, `expo-dev-client`, `expo-splash-screen`, `expo-updates`, `expo-notifications`, `react`, `react-native`, `react-native-safe-area-context`, `react-native-screens`, `@assistant-ui/react-native`. Add `@react-native-async-storage/async-storage` only when something must persist; nothing does in v1.
