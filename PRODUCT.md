...
- **The app is one surface**: a chat thread with the coach, plus push notifications. It stores nothing. Verdicts written while the app was closed appear when it comes to the foreground.
...
- The thread streams the coach's reply. History loads on mount and on foreground. Intake happens in the thread; there is no separate onboarding screen.
